...
namespace PlaylistDownloader
{
    public class Downloader : BackgroundWorker
    {
        ...
        {
            try {
                PlaylistLogger.Info("Start Downloading " + item.Name);
                ...
                PlaylistLogger.Info("Finished Downloading " + item.Name);

                return destinationFilePathWithoutExtension;
            } catch(Exception ex) {
                var serializer = new JsonSerializer();
                PlaylistLogger.Error("Failed to download item: " + serializer.Serialize(ex));
                return null;
            }
        }
